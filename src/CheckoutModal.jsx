...
export default function CheckoutModal({ open, onClose, course}) {
  ...
  const handleProceed = async () => {
    ...
    console.log(order,user,course)
    if(order.status==="Failed"){
        alert("Cource Already purcheshed");
    }

    else{
          openRazorPayPopup(order.orderId,user,course,onClose);
    }
  };
      ...
      >
        ...
        <div className="p-5 border-t border-gray-200 dark:border-gray-800 flex gap-3">
          ...
          <button
            ...
            disabled={!isValid}
            className={`flex-1 h-11  rounded-lg text-white  transition-colors ${isValid
...
const openRazorPayPopup=(orderId,user,course,onClose)=>{
  console.log("user",course)
    ...
    name:"Satyam Shukla",
    order_id:orderId,
    image:"http://172.21.82.145:5500/procodrr.png",
    theme:{
        color:'#7FFF00'
    },
    prefill: {
    ...
    handler:async(response)=> {
      ...
      const comorderResponse = await fetch("http://localhost:4000/complete-order", {
      method: 'POST',
      body: JSON.stringify(payload),
      headers:{
        'Content-Type':'application/json'
